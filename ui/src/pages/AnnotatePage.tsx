import { Box as BoxIcon, ChevronDown, ChevronLeft, ChevronRight, Copy, Layers, LayoutGrid, Move, RotateCcw, Save, Search, Settings2, Trash2, X, Zap } from 'lucide-react';
import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { useNavigate, useParams, useSearchParams } from 'react-router-dom';
import { API } from '../api';
import { Annotator } from '../components/canvas/Annotator';
import { BBox, Sample, Waypoint } from '../types';
...
export const AnnotatePage: React.FC = () => {
  ...
  const handleUpdate = useCallback((updated: Partial<Sample>) => {
    ...
    setCurrentData(prev => ({ ...prev, ...finalUpdate } as CurrentDataState));
  }, [currentData, lastSelectedClass]);
      ...
      <header className="h-16 border-b border-white/20 flex items-center justify-between px-8 bg-[#0a0a0c] z-50 shadow-xl">
        <div className="flex items-center gap-8">
            ...
            <div className="flex flex-col">
                <h1 className="text-sm font-cyber tracking-[0.25em] text-accent uppercase font-black">NeuroPilot_Extreme_v4</h1>
                <p className="text-base text-white font-mono font-black">{selectedFilename || 'AWAITING_INPUT...'}</p>
            </div>
        </div>
      ...
      <div className="flex-1 relative flex overflow-hidden">
        <aside className="w-72 border-r border-white/20 bg-[#0a0a0c] flex flex-col z-40 shadow-2xl">
             <div className="p-5 border-b border-white/10 flex items-center gap-3 bg-white/10">
                ...
                <span className="text-sm font-cyber text-white font-black uppercase tracking-wider">Active Layers</span>
             </div>
             <div className="flex-1 overflow-y-auto p-3 space-y-2 cyber-scrollbar">
                {currentData.bboxes?.map((b, i) => (
                    <div key={i} onClick={() => setSelectedBBoxIdx(i)} className={`p-3.5 rounded-2xl flex items-center gap-4 cursor-pointer transition-all ${selectedBBoxIdx === i ? 'bg-accent/20 border-2 border-accent/50 shadow-lg scale-[1.02]' : 'bg-white/[0.05] border border-white/10 hover:bg-white/10'}`}>
                        ...
                        <span className={`text-sm font-black truncate flex-1 ${selectedBBoxIdx === i ? 'text-white' : 'text-white'}`}>{classes[b.category] || `ID_${b.category}`}</span>
                        <Trash2 onClick={(e) => {
                            e.stopPropagation();
                            const next = [...currentData.bboxes!];
                            next.splice(i, 1);
                            handleUpdate({ bboxes: next });
                            if (selectedBBoxIdx === i) {
                                setSelectedBBoxIdx(null);
                            } else if (selectedBBoxIdx !== null && selectedBBoxIdx > i) {
                                setSelectedBBoxIdx(selectedBBoxIdx - 1);
                            }
                        }} className="w-4 h-4 text-white/40 hover:text-red-500 transition-colors" />
                    </div>
                ))}
                {currentData.waypoints?.length === 10 && (
                    <div onClick={() => setMode('waypoint')} className={`p-3.5 rounded-2xl flex items-center gap-4 cursor-pointer transition-all ${mode === 'waypoint' ? 'bg-green-500/20 border-2 border-green-500/50 shadow-lg scale-[1.02]' : 'bg-white/[0.05] border border-white/10 hover:bg-white/10'}`}>
                        ...
                        <span className={`text-sm flex-1 font-black ${mode === 'waypoint' ? 'text-white' : 'text-white'}`}>Unified Path (10 Points)</span>
        ...
        <aside className="w-80 border-l border-white/20 bg-[#0a0a0c] flex flex-col p-7 space-y-12 z-40 overflow-y-auto cyber-scrollbar">
            <section className="space-y-5">
                <h3 className="text-sm text-white font-black uppercase tracking-[0.2em] font-cyber border-l-4 border-accent pl-4">Tool Configuration</h3>
                <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => { setMode('bbox'); setSelectedBBoxIdx(null); }} className={`flex flex-col items-center gap-3 p-6 rounded-3xl border-2 transition-all ${mode === 'bbox' ? 'bg-accent/20 border-accent text-white shadow-[0_0_20px_rgba(0,255,65,0.2)]' : 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10'}`}>
                        ...
                        <span className="text-sm font-black tracking-wider uppercase">Objects</span>
                    </button>
                    <button onClick={() => { setMode('waypoint'); setSelectedBBoxIdx(null); }} className={`flex flex-col items-center gap-3 p-6 rounded-3xl border-2 transition-all ${mode === 'waypoint' ? 'bg-accent/20 border-accent text-white shadow-[0_0_20px_rgba(0,255,65,0.2)]' : 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10'}`}>
                        ...
                        <span className="text-sm font-black tracking-wider uppercase">Path</span>
                    </button>
                </div>
            </section>

            <section className="space-y-5">
                <h3 className="text-sm text-white font-black uppercase tracking-[0.2em] font-cyber border-l-4 border-accent pl-4 flex items-center gap-3"><Zap className="w-4 h-4 text-accent fill-accent" /> Path Templates</h3>
            ...
            <section className="flex-1 flex flex-col min-h-0 space-y-5">
                <div className="flex justify-between items-center">
                    <h3 className="text-sm text-white font-black uppercase tracking-[0.2em] font-cyber border-l-4 border-accent pl-4">Classification</h3>
                ...
                <div className="relative">
                    <input
                        ...
                        className="w-full bg-white/10 border-2 border-white/20 rounded-2xl p-4 pl-12 text-base text-white font-black placeholder:text-white/30 focus:border-accent outline-none transition-all shadow-inner"
                    />
                ...
                <div className="flex-1 overflow-y-auto space-y-2 pr-2 cyber-scrollbar max-h-64">
                    {filteredClasses.map((c) => (
                        ...
                        >
                            ...
                            <span className="text-base font-black truncate text-left flex-1 uppercase tracking-wider">{c.id}: {c.name}</span>
...
const TemplateBtn = ({ label, onClick }: { label: string, onClick: () => void }) => (
    <button onClick={onClick} className="p-5 bg-white/[0.03] hover:bg-white/10 rounded-3xl text-sm font-black border border-white/10 hover:border-accent transition-all flex justify-between items-center text-white group shadow-2xl">
