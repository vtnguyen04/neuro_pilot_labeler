...
from ...domain.interfaces.repository import IProjectRepository, ISampleRepository
from ...domain.interfaces.storage import IStorageProvider
from ...utils.yolo_utils import save_yolo_label
from ..interfaces.repository import IVersionRepository
...
class VersionService:
    ...
    def publish_version(
        ...
        classes = self.project_repo.get_project(project_id).classes if self.project_repo.get_project(project_id) else []
        if not classes:
            classes = [
                "Stop",
                "Obstacle",
        ...
        def process_item(item):
            ...
            data_bytes = None
            # Check if we need to fetch from storage provider
            if img_path and (img_path.startswith("minio://") or not Path(img_path).exists()):
                try:
                    # In DDD we use resolve_file_path to get bytes or path
                    resolved = self.storage.resolve_file_path(filename, img_path)
                    ...
                    print(f"Error resolving {img_path}: {e}")
                    return None
            else:
                src_img = Path(img_path)

            # Handle Copy or Resize
            if data_bytes:
                try:
                    ...
                    print(f"Error processing image bytes {filename}: {e}")
                    return None
            else:
                if src_img.exists():
                    if resize_width is None and resize_height is None:
                        ...
                        try:
                            ...
                            print(f"Error resizing image file {filename}: {e}")
                            return None
                else:
        ...
        yaml_content = {"path": str(version_dir.absolute()), "nc": len(classes), "names": classes}
        if train_samples:
